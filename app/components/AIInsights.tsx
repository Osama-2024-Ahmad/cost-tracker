...
function AIInsights({ refreshTrigger }: AIInsightsProps) {
  ...
  return (
    <div className="col-span-1 md:col-span-2 p-6 shadow-blue-100 shadow-md rounded-xl bg-white mt-5">
      {/* Card Header */}
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-blue-600 font-medium">{lastUpdated}</span>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {insights.map((insight) => (
            <div
              key={insight.id}
              className={`p-5 rounded-xl ${getInsightColor(
                insight.type
